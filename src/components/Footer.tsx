...
export default function Footer({ tr }: Props) {
  const lang = tr.nav.langLabel === "FR" ? "en" : "fr";

  const services = lang === "en"
    ? ["Geophysical prospection", "Hydrogeological study", "Electrical tomography", "Drilling report", "Pre-drilling guidance"]
    : ["Prospection géophysique", "Étude hydrogéologique", "Tomographie électrique", "Rapport de forage", "Accompagnement avant forage"];

  const navLinks = [
    { label: tr.whoWeAre.label, href: "#qui-sommes-nous" },
    { label: tr.method.label,   href: "#methode" },
    { label: tr.nav.report,     href: "#rapport" },
    { label: lang === "en" ? "Morocco Map" : "Carte Maroc", href: "#carte" },
    { label: tr.references.title.split(" ").slice(0,2).join(" "), href: "#references" },
    { label: tr.contact.label,  href: "#contact" },
  ];

  const copyright = lang === "en"
    ? `© ${new Date().getFullYear()} Allo Sources — All rights reserved`
    : `© ${new Date().getFullYear()} Allo Sources — Tous droits réservés`;

  const disclaimer = lang === "en"
    ? "The estimates presented reduce drilling risks without constituting an absolute guarantee of results."
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-14 pb-10">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <p className="text-white/40 text-sm leading-relaxed mb-5">{tr.footer.tagline}</p>
            ...
            <ul className="space-y-3 text-sm text-white/50">
              <li>
                <a href={`tel:${PHONE}`} className="flex items-start gap-2.5 hover:text-white/80 transition-colors">
                  <Phone className="w-4 h-4 text-water/60 flex-shrink-0 mt-0.5"/>{PHONE}
                </a>
              </li>
              <li>
                <a href={`mailto:${EMAIL}`} className="flex items-start gap-2.5 hover:text-white/80 transition-colors">
                  <Mail className="w-4 h-4 text-water/60 flex-shrink-0 mt-0.5"/>{EMAIL}
                </a>
              </li>
              <li className="flex items-start gap-2.5">
                <MapPin className="w-4 h-4 text-water/60 flex-shrink-0 mt-0.5"/>
                <span>
                  {tr.contact.locationCity}
                  <br/><span className="text-white/30 text-xs">{tr.contact.locationSub}</span>
                </span>
              </li>
            </ul>
          </div>
